...
DEFAULT_DARK_COLOR = 'gray20'
DEFAULT_INFORMATIVE_WINDOW_COLOR = 'grey93'
...
def pop_up_plot_window(path):
    """
    The function will pop up a new window and will show the plot of the correct
    graph image
    Args:
...
class Gui_3D_Bio:
    ...
    def __init__(self, root):
        ...
        self._middleBottomFrame = Frame(root, width=770, height=650,
                                        bg=DEFAULT_INFORMATIVE_WINDOW_COLOR)
    ...
    def normalLabelCreateAndPack(self, frame, side, text, font, bg, fg):
        """
        The method will create new label, pack it and add it to the normal
        labels array
        Args:
    ...
    def informativeLabelCreateAndPack(self, frame, text, fg, bg, font, side):
        """
        The method will create new label, pack it and add it to the informative
        labels array
        Args:
            frame: given frame to pack in as Frame
            text: the label's string as string
    ...
    def buttonCreateAndPack(self, frame, text, height, width, font, bg,
                            command, side):
        """
        The method will create new button, pack it and add it to the buttons
        array
        Args:
            frame: given frame to pack in as Frame
        ...
        """
        button = Button(frame, text=text, height=height, width=width,
                        font=font, bg=bg, command=command)
    ...
    def labelFrameCreation(self, frame, text, fg, bg, font, side):
        """
        The method will create new labelFrame, pack it and add it to the
        labelFrames array
        Args:
            frame: given frame to pack in as Frame
    ...
    def entryCreateAndPack(self, frame, textVariable, bd, side):
        """
        The method will create new entry, pack it and add it to the entries
        array
        Args:
            frame: given frame to pack in as Frame
    ...
    def checkBoxCreateAndPack(self, frame, text, variable, bg, fg, font, side):
        """
        The method will create new checkBox, pack it and add it to the
        checkBoxes array
        Args:
            frame: given frame to pack in as Frame
        ...
        """
        checkBox = Checkbutton(frame, text=text, variable=variable, bg=bg,
                               fg=fg, font=font)
    ...
    def spaceCreation(self, frame, amount, side, bg):
        """
        The method will create new empty label, pack it and add it to the
        informative labels array
        Args:
            frame: given frame to pack in as Frame
    ...
    def titleCreation(self):
        ...
        """
        self.normalLabelCreateAndPack(self._titleFrame, DEFAULT_DIRECTION,
                                      'Membraneless Organelles Final Project',
                                      ("Linux Libertine Mono O", 15, 'bold'),
    ...
    def changeModeAndExitButton(self):
        """
        The method will changeMode and Exit buttons (included pack + add)
        Returns: None

        """
        self.spaceCreation(self._leftButtonsFrame, 2, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        self.buttonCreateAndPack(self._leftButtonsFrame, 'Dark/Light Mode',
                                 1, 15,
                                 (DEFAULT_FONT, DEFAULT_FONT_SIZE, 'bold'),
                                 DARK_MODE_DEFAULT_FONT_COLOR,
                                 lambda: self.darkLightSwitch(),
                                 DEFAULT_DIRECTION)

        self.spaceCreation(self._leftButtonsFrame, 3, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        self.buttonCreateAndPack(self._leftButtonsFrame, 'Exit', 1, 15,
                                 (DEFAULT_FONT, DEFAULT_FONT_SIZE, 'bold'),
                                 DARK_MODE_DEFAULT_FONT_COLOR,
                                 lambda: quit(), DEFAULT_DIRECTION)

    def gridTopFrameEntry(self):
        """
        The method will pack all the relevant entry places(included pack + add)
        Returns: None

        """
        entryLabelFrame = self.labelFrameCreation(self._middleTopFrame,
                                                  'Program Input',
                                                  DEFAULT_FONT_COLOR,
                                                  FRAME_COLOR,
                                                  (DEFAULT_FONT,
        ...
        self.normalLabelCreateAndPack(entryLabelFrame, DEFAULT_DIRECTION,
                                      'Enter Sequence:',
                                      (DEFAULT_FONT, 9, 'bold'),
                                      FRAME_COLOR, DEFAULT_FONT_COLOR)

        self.entryCreateAndPack(entryLabelFrame, self._givenSequence, 2,
                                DEFAULT_DIRECTION)

        self.spaceCreation(entryLabelFrame, 1, DEFAULT_DIRECTION, FRAME_COLOR)

        self.buttonCreateAndPack(entryLabelFrame, 'Start Simulation', 1, 15,
                                 (DEFAULT_FONT, DEFAULT_FONT_SIZE, 'bold'),
                                 DARK_MODE_DEFAULT_FONT_COLOR,
                                 lambda: self.runSimulation(),
                                 DEFAULT_DIRECTION)
    ...
    def runSimulation(self):
        ...
        if not self.checkValidity():
            self.informativeLabelCreateAndPack(self._middleBottomFrame,
                                               'Invalid Input Given.'
                                               ' Hint: Negative Argument',
                                               'blue',
                                               DEFAULT_INFORMATIVE_WINDOW_COLOR
                                               , (
                                                   DEFAULT_FONT,
                                                   DEFAULT_FONT_SIZE,
                                                   'normal'),
        ...
        self.informativeLabelCreateAndPack(self._middleBottomFrame,
                                           'Simulating is Over, Check Out'
                                           ' Your Plots',
                                           'blue',
                                           DEFAULT_INFORMATIVE_WINDOW_COLOR, (
                                               DEFAULT_FONT, DEFAULT_FONT_SIZE,
                                               'normal'), DEFAULT_DIRECTION)
    ...
    def configureFramesToDark(self):
        ...
        """
        self._titleFrame.configure(background=DEFAULT_DARK_COLOR)
        self._leftButtonsFrame.configure(background=DEFAULT_DARK_COLOR)
        self._rightButtonsFrame.configure(background=DEFAULT_DARK_COLOR)
        self._middleTopFrame.configure(background=DEFAULT_DARK_COLOR)
        self._lowBorder.configure(background=DEFAULT_DARK_COLOR)
    ...
    def configureWidgetsToDark(self):
        ...
        for label in self._allLabels:
            label.config(bg=DEFAULT_DARK_COLOR)
    ...
    def gridPlotCheckBoxes(self):
        """
        The method will pack all the relevant checkBoxes places
        (included pack + add)
        Returns: None

        """
        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Select Your Plots:',
                                      (DEFAULT_FONT, 9, 'normal'),
                                      bg=FRAME_COLOR, fg=DEFAULT_FONT_COLOR)

        self.spaceCreation(self._rightButtonsFrame, 1, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        self.checkBoxCreateAndPack(self._rightButtonsFrame, 'Energy Over Time',
                                   self._firstCheckBoxStatus,
                                   FRAME_COLOR, CHECKBOX_DEFAULT_FONT_COLOR,
                                   (DEFAULT_FONT, 7, 'normal'),
                                   DEFAULT_DIRECTION)

        self.checkBoxCreateAndPack(self._rightButtonsFrame,
                                   'End to End Distances',
                                   self._secondCheckBoxStatus,
                                   FRAME_COLOR, CHECKBOX_DEFAULT_FONT_COLOR,
                                   (DEFAULT_FONT, 7, 'normal'),
                                   DEFAULT_DIRECTION)
        ...
        self.checkBoxCreateAndPack(self._rightButtonsFrame,
                                   ...
                                   self._fourthCheckBoxStatus,
                                   FRAME_COLOR, CHECKBOX_DEFAULT_FONT_COLOR,
                                   (DEFAULT_FONT, 7, 'normal'),
                                   DEFAULT_DIRECTION)

        self.checkBoxCreateAndPack(self._rightButtonsFrame, 'Beads Location',
                                   self._fifthCheckBoxStatus, FRAME_COLOR,
    ...
    def gridArguments(self):
        """
        The method will pack all the relevant arguments (included pack + add)
        Returns: None

        """
        kInLabelFrame = self.labelFrameCreation(self._middleTopFrame,
                                                'Argument 1:',
                                                DEFAULT_FONT_COLOR,
                                                FRAME_COLOR,
                                                (DEFAULT_FONT,
        ...
        kOutLabelFrame = self.labelFrameCreation(self._middleTopFrame,
                                                 'Argument 2:',
                                                 DEFAULT_FONT_COLOR,
                                                 FRAME_COLOR,
                                                 (DEFAULT_FONT,
        ...
        self.normalLabelCreateAndPack(kInLabelFrame, 'left', "K-In:",
                                      (DEFAULT_FONT, 9, 'bold'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.entryCreateAndPack(kInLabelFrame, self._kInSize, 2, 'left')

        self.normalLabelCreateAndPack(kOutLabelFrame, 'left', "K-Out:",
                                      (DEFAULT_FONT, 9, 'bold'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)
    ...
    def gridLeftSideWidgets(self):
        """
        The method will pack all the relevant widgets which on the left side
        (included pack + add)
        Returns: None

        """
        entryLabelFrame = self.labelFrameCreation(self._leftButtonsFrame,
        ...
        self.entryCreateAndPack(entryLabelFrame, self._locationToSave, 2,
                                DEFAULT_DIRECTION)

        self.spaceCreation(self._leftButtonsFrame, 1, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        beadRadiusLabelFrame = self.labelFrameCreation(self._leftButtonsFrame,
                                                       'Argument 3:',
                                                       DEFAULT_FONT_COLOR,
                                                       FRAME_COLOR, (
                                                           DEFAULT_FONT,
        ...
        self.normalLabelCreateAndPack(beadRadiusLabelFrame, DEFAULT_DIRECTION,
                                      "Enter Bead Radius:",
                                      (DEFAULT_FONT, 9, 'bold'),
                                      FRAME_COLOR, DEFAULT_FONT_COLOR)

        self.entryCreateAndPack(beadRadiusLabelFrame, self._beadRadiusSize, 2,
                                DEFAULT_DIRECTION)

        self.spaceCreation(self._leftButtonsFrame, 1, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        sphereRadiusLabelFrame = self.labelFrameCreation(
            self._leftButtonsFrame, 'Argument 4:', DEFAULT_FONT_COLOR,
            FRAME_COLOR, (DEFAULT_FONT, DEFAULT_FONT_SIZE, 'normal'),
            DEFAULT_DIRECTION)

        self.normalLabelCreateAndPack(sphereRadiusLabelFrame,
                                      DEFAULT_DIRECTION,
                                      "Enter Sphere Radius:",
                                      (DEFAULT_FONT, 9, 'bold'),
                                      FRAME_COLOR, DEFAULT_FONT_COLOR)

        self.entryCreateAndPack(sphereRadiusLabelFrame, self._sphereRadiusSize,
                                2, DEFAULT_DIRECTION)

        self.spaceCreation(self._leftButtonsFrame, 1, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        kbsLabelFrame = self.labelFrameCreation(self._leftButtonsFrame,
                                                'Argument 5:',
                                                DEFAULT_FONT_COLOR,
                                                FRAME_COLOR,
                                                (DEFAULT_FONT,
        ...
        self.normalLabelCreateAndPack(kbsLabelFrame, DEFAULT_DIRECTION,
                                      "Enter KBS:", (DEFAULT_FONT, 9, 'bold'),
                                      FRAME_COLOR, DEFAULT_FONT_COLOR)

        self.entryCreateAndPack(kbsLabelFrame, self._kbsValue, 2,
                                DEFAULT_DIRECTION)

        self.spaceCreation(self._leftButtonsFrame, 1, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        chainsNumberLabelFrame = self.labelFrameCreation(
            self._leftButtonsFrame, 'Argument 6:', DEFAULT_FONT_COLOR,
            FRAME_COLOR, (DEFAULT_FONT, DEFAULT_FONT_SIZE, 'normal'),
            DEFAULT_DIRECTION)

        self.normalLabelCreateAndPack(chainsNumberLabelFrame,
                                      DEFAULT_DIRECTION,
                                      "Enter Number Of Chains:",
                                      (DEFAULT_FONT, 9, 'bold'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.entryCreateAndPack(chainsNumberLabelFrame, self._chainsNumber, 2,
                                DEFAULT_DIRECTION)

        self.spaceCreation(self._leftButtonsFrame, 1, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        aminoNumberLabelFrame = self.labelFrameCreation(self._leftButtonsFrame,
                                                        'Argument 7:',
                                                        DEFAULT_FONT_COLOR,
                                                        FRAME_COLOR, (
                                                            DEFAULT_FONT,
        ...
        self.normalLabelCreateAndPack(aminoNumberLabelFrame, DEFAULT_DIRECTION,
                                      "Amino Acid Number:",
                                      (DEFAULT_FONT, 9, 'bold'),
                                      FRAME_COLOR, DEFAULT_FONT_COLOR)

        self.entryCreateAndPack(aminoNumberLabelFrame, self._aminoAmount, 2,
                                DEFAULT_DIRECTION)

        self.spaceCreation(self._leftButtonsFrame, 1, DEFAULT_DIRECTION,
                           FRAME_COLOR)

    def gridInstructionsAndAuthors(self):
        """
        The method will pack all the basic instructions (included pack + add)
        Returns: None

        """
        self.spaceCreation(self._rightButtonsFrame, 2, DEFAULT_DIRECTION,
                           FRAME_COLOR)

        randomInitLabelFrame = self.labelFrameCreation(self._rightButtonsFrame,
                                                       'Argument 8:',
                                                       DEFAULT_FONT_COLOR,
                                                       FRAME_COLOR, (
                                                           DEFAULT_FONT,
        ...
        self.checkBoxCreateAndPack(randomInitLabelFrame, 'Is Centered?',
                                   self._randomInitCheckBoxStatus,
                                   FRAME_COLOR, CHECKBOX_DEFAULT_FONT_COLOR,
                                   (DEFAULT_FONT, 7, 'normal'),
                                   DEFAULT_DIRECTION)
        ...
        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Quick Instructions:',
                                      (DEFAULT_FONT, DEFAULT_FONT_SIZE,
                                       'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Step One:',
                                      (DEFAULT_FONT, DEFAULT_FONT_SIZE,
                                       'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)
        ...
        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION,
                                      'Fill All The Arguments 1-8',
                                      (DEFAULT_FONT, 7, 'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Step Two:',
                                      (DEFAULT_FONT, DEFAULT_FONT_SIZE,
                                       'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Fill Your File Name',
                                      (DEFAULT_FONT, 7, 'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Step Three:',
                                      (DEFAULT_FONT, DEFAULT_FONT_SIZE,
                                       'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Fill Your Sequence',
                                      (DEFAULT_FONT, 7, 'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Step Four:',
                                      (DEFAULT_FONT, DEFAULT_FONT_SIZE,
                                       'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)
        ...
        self.normalLabelCreateAndPack(self._rightButtonsFrame,
                                      DEFAULT_DIRECTION, 'Enjoy!',
                                      (DEFAULT_FONT, DEFAULT_FONT_SIZE,
                                       'normal'), FRAME_COLOR,
                                      DEFAULT_FONT_COLOR)

        self.normalLabelCreateAndPack(self._rightButtonsFrame, 'bottom',
                                      'Ilia Bezgin',
        ...
        self.normalLabelCreateAndPack(self._rightButtonsFrame, 'bottom', 'By:',
                                      (DEFAULT_FONT, 6, 'normal'),
                                      FRAME_COLOR, DEFAULT_FONT_COLOR)
...
def gui_buttons_and_design_init(program):
    """
    The method will setup all the buttons and the general design at the given
    program window
    Args:
    ...
    program.gridTopFrameEntry()
    program.gridArguments()
    program.gridLeftSideWidgets()
    program.gridInstructionsAndAuthors()
